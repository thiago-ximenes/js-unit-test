...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    expect(createMenu({ name: 'Refri', product: 'Burgão'}).fetchMenu()).toEqual({name: 'Refri', product: 'Burgão'})
    ...
    // ```
      ...
      expect(createMenu().order('coxinha') && createMenu('Comida', 'Bebida').consumption).toEqual(['coxinha']);
    ...
    expect(
      ...
      createMenu().consumption).toEqual(['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    createMenu().consumption).toEqual(['coxinha', 'agua', 'coxinha']);
    // ```
    ...
    ).toEqual(11.7); // Retorno: somaDosPreçosDosPedidos
    // ```
